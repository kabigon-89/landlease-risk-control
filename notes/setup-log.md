...
## 2026-09-20
- 設計を簡略化：仕様書の「1次・2次通知＋Approval機能」を、「更新確認」1件のレコードが所管課から資産経営課へ担当を移す構成に変更。Task拡張・Flow Designerは使わず、独立テーブル＋スクリプト＋UI Actionで実装
- 「更新確認」テーブルを新規作成（自動採番REN。契約案件・対象契約バージョン・ステータス・担当グループ・所管課の判断・コメント・回答日時）
- 契約案件に「所管課」項目を追加。グループ「資産経営課」「道路公園課」を作成（デモ用）
- 期日検知：Script Include「RenewalCheckCreator」＋日次Scheduled Script（毎日6時）。現行バージョンのみ、契約終了日の（事前通知月数＋3か月）前に自動起票。二重起票防止あり（デモは事前通知月数=6を手入力）
- 所管課の回答：フォーム上のボタン「契約更新／契約変更／契約終了」。回答で担当グループが資産経営課に切り替わる
- 資産経営課側：「前回契約書をダウンロード」（Word優先、なければPDF）、「契約作業画面を開く」（現行バージョンで開く）、「事務処理を完了」
- 「案文確認」テーブルを新規作成（自動採番DFT）。所管課ボタン「案文確認済み」「案文差し戻し（理由必須）」
- 依頼はリスク抽出画面のボタン＋ポップアップ（依頼コメント任意）。未確認のリスク判定が残っている場合は警告を表示
- 差し戻し時は、リスク抽出画面に所管課の差し戻し理由を黄色枠で表示。再依頼が可能
- 「事務処理を完了」は、案文確認が「確認済み」になるまで押せない（契約終了の場合は不要）
- 契約作業ワークスペースの経緯メモに所管課の回答を自動入力（二重入力を防止）。入力欄を拡大
- ポータルに「ホーム」（landlease_home）を新規作成。所管課／資産経営課の表示をデモ用に切替（URLの`mode=asset`でも指定可）
- 所管課：新着通知カード（更新可否の確認・契約書案の確認）。資産経営課：対応が必要な案件と所管課の回答待ち。件数サマリー付き
- 各フォームとリスク抽出画面に「ホームに戻る」を追加。回答・完了ボタンの押下後はホームへ遷移
- 期日到来の通知 → 所管課回答 → 資産経営課対応 → 契約書チェック → 案文確認依頼まで通しで確認
- 案文確認の差し戻し・再依頼・確認済み、事務処理完了の条件制御を確認

## 2026-09-21
- 処理時間の対策として、別紙（添付資料）は契約全体レベルの判定（③添付文書との相違）にのみ渡し、条文単位の判定には渡さない形に変更。金額検算が失敗しても全体の処理は止めず、警告を出して続行する
- 判定モデルをgpt-5-miniからgpt-5.6-terraに変更（aoai-landlease-poc、グローバル標準）。
- AI判定を並列化（ThreadPoolExecutor、同時実行4件）。契約全体の判定・金額検算・各条文の判定を並列に実行し、ServiceNowへの書き込みは条文の順番どおりに直列で行う。利用枠（429）対策としてSDKの再試行を5回に増やし、処理時間をログに出力。処理時間は約6分から約40〜50秒に短縮
- 金額検算のバグを修正。プロンプトが年額を計算するコードを組み立てさせていたため、賃料が月額で記載された契約書では、4,500,000円と400,000円を比べて差異4,100,000円と誤検出していた。検算結果の単位を契約書記載額と揃えるよう指示を修正し、375,000円と400,000円の差異-25,000円を確認
- 金額検算で差異が出た場合は、第0条（契約全体）に「③他の情報源との矛盾」の指摘として登録し、画面のカードに表示する。あわせて、AI判定側の指示を「金額の食い違いは金額検算が担当し、AIは金額以外（面積・期間・営業時間等）を対象とする」に変更し、同じ内容が2枚並ばないようにした
- 契約バージョンの番号の付け方を修正。開いたバージョンの番号+1だったため、古いバージョンから作ると番号が重複していた。同じ契約案件内の最大の番号+1に変更。あわせて既存の現行バージョンの番号を1から3に直し、次のバージョンが4になることを確認
- 履歴ビューア（ナレッジ蓄積）を実装。ウィジェットとページ（history_viewer）を作成し、契約一覧→バージョンの時系列→バージョン詳細を1つのウィジェット内で切り替える。詳細には、基本情報・経緯メモ・所管課とのやり取り（更新確認・案文確認。案文確認のきっかけになった更新確認も表示）・リスク判定（「要修正」またはメモありの指摘のみ。AI検出件数に対する表示件数も併記）を表示。